<x-app-layout>
    ...
    <x-slot name="buttons">
        @if (count($customers) > 0)
            @can(Permissions::getPermissionKey(Permissions::CREATE_CUSTOMER))
                <x-button tag="a" href="{{ route('customers.create') }}">Neuer Kunde</x-button>
            @endcan
        @endif
    </x-slot>
    ...
    <div class="w-full h-auto py-12">
                                    ...
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th scope="col"
                                                ...
                                                Firmenname</th>
                                            <th scope="col"
                                                class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">PLZ
                                            <th scope="col"
                                                class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Ort
                                            </th>
                                            <th scope="col"
                                                class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">E-Mail
                                            </th>
                                            <th scope="col"
                                    ...
                                    <tbody class="divide-y divide-gray-200 bg-white">
                                        @foreach ($customers as $customer)
                                            <tr>
                                                ...
                                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    {{ $customer->contact_email }}</td>
                                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    {{ $customer->contact_phone }}</td>
                                                <td
        ...
        @else
            ...
            <x-empty-state headline="Keine Kunden" copy="Beginnen Sie mit dem Anlegen eines neuen Kunden."
                buttonLabel="Neuer Kunde" buttonLink="{{ route('customers.create') }}" :permissionKey="Permissions::CREATE_CUSTOMER" />
        @endif
    </div>
</x-app-layout>
