<x-app-layout>
    ...
    <x-slot name="buttons">
        @if (count($subsections) > 0)
            @can(Permissions::getPermissionKey(Permissions::CREATE_SUBSECTION))
                <x-button tag="a" href="{{ route('subsections.create') }}">Neues Gewerk</x-button>
            @endcan
        @endif
    </x-slot>
    ...
    <div class="w-full h-auto py-12">
                                        ...
                                        @foreach ($subsections as $subsection)
                                            <tr>
                                                ...
                                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    {{ $subsection->description }}</td>
                                                <td
                                                    class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                    @can(Permissions::getPermissionKey(Permissions::READ_SUBSECTION))
                                                        <a href="{{ route('subsections.show', ['id' => $subsection->id]) }}"
        ...
        @else
            <x-empty-state headline="Keine Anfragen" copy="Beginnen Sie mit dem Anlegen eines neue Anfrage."
                buttonLabel="Neue Anfrage" buttonLink="{{ route('subsections.create') }}" :permissionKey="Permissions::CREATE_SUBSECTION" />
        @endif
    </div>
</x-app-layout>
