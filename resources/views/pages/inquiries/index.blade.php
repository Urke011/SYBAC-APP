    ...
    <x-slot name="buttons">
        @if (count($inquiries) > 0)
            @can(Permissions::getPermissionKey(Permissions::CREATE_INQUIRY))
                <x-button tag="a" href="{{ route('inquiries.create') }}">Neue Anfrage</x-button>
            @endcan
        @endif
    </x-slot>
    ...
    <div class="w-full h-auto py-12">
                                            ...
                                            <tr>
                                                @can(Permissions::getPermissionKey(Permissions::READ_CUSTOMER))
                                                    <td
                                                        ...
                                                        {{ $inquiry->customer->company_name }}</td>
                                                @endcan
                                                ...
                                                <td
                                                    ...
                                                    @can(Permissions::getPermissionKey(Permissions::READ_INQUIRY))
                                                        <a href="{{ route('inquiries.show', ['id' => $inquiry->id]) }}"
        ...
        @else
            <x-empty-state headline="Keine Anfragen" copy="Beginnen Sie mit dem Anlegen eines neue Anfrage."
                buttonLabel="Neue Anfrage" buttonLink="{{ route('inquiries.create') }}" :permissionKey="Permissions::CREATE_INQUIRY" />
        @endif
    </div>
</x-app-layout>
